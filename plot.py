...
def plotAccuracy(
    results, truth, train=None, window=100, label=None, params=None, errorType=None
):
...
def computeAltMAPE(truth, prediction, startFrom=0):
    return np.nanmean(np.abs(truth[startFrom:] - prediction[startFrom:])) / np.nanmean(
...
class ExperimentResult(object):
    ...
    def loadExperiment(self, experiment):
        ...
        if params["output_encoding"] == "likelihood":
            ...
            self.outputEncoder = NupicScalarEncoder(
                w=1, minval=0, maxval=40000, n=22, forced=True
            )
    ...
    def computeError(self):
        if self.params["output_encoding"] == "likelihood":
            ...
            self.error = computeLikelihood(
                self.predictions, self.truth, self.outputEncoder
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "experiments", metavar="/path/to/experiment /path/...", nargs="+", type=str
    )
