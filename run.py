...
def _getArgs():
    parser = OptionParser(
        ...
        "and its application to sequence tagging,"
        "with the CONLL-2003 named entity recognition task."
    )
    parser.add_option(
        "-d",
        "--dataSet",
        ...
        default="conll-2003",
        dest="dataSet",
        help="DataSet Name, own choice of conll-2003",
    )
    parser.add_option(
        "-l",
        "--numLags",
...
def initializeNet(
    ...
    assert (
        algorithm == "FOSELM"
        or algorithm == "NFOSELM"
        or algorithm == "NAOSELM"
        or algorithm == "ORELM"
    )
    if algorithm == "FOSELM":
        """
...
def readDataSet(dataSet):
    ...
    if dataSet == "nyc_taxi":
        ...
        seq = pd.DataFrame(
            np.array(pd.concat([sequence, timeofday, dayofweek], axis=1)),
...
if __name__ == "__main__":
  ...
  """
    ...
    saveResultToFile(
        dataSet,
        predictedInput,
        "FF" + str(net.forgettingFactor) + algorithm + str(net.numHiddenNeurons),
        predictionStep,
    )
  ...
  """
    ...
    targetPlot, = plt.plot(
        targetInput, label="target", color="red", marker=".", linestyle="-"
    )
    predictedPlot, = plt.plot(
        predictedInput, label="predicted", color="blue", marker=".", linestyle=":"
    )
    ...
    plt.title(
        "Time-series Prediction of " + algorithm + " on " + dataSet + " dataset",
        fontsize=20,
        fontweight=40,
    )
