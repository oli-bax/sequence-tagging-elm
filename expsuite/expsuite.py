...
class PyExperimentSuite(object):
    ...
    def get_values_fix_params(self, exp, rep, tag, which="last", **kwargs):
        ...
        values = [
            ...
            for se in subexps
            if all(map(lambda tv: tv in se, tagvalues))
        ]
        params = [
            self.get_params(se)
            for se in subexps
        ...
        for d in self.get_exps("."):
            ...
            try:
                ...
                print(
                    "         started %s"
                    % time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(os.stat(minfile).st_mtime)
                    )
                )
                print(
                    "           ended %s"
                    % time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(os.stat(maxfile).st_mtime)
                    )
                )
            ...
            if "experiment" in params and params["experiment"] == "single":
                ...
                if len(iterparams) > 0:
                    ...
                    for il in iterfunc(*[params[p] for p in iterparams]):
                        par = params.copy()
                        converted = str(
                            zip(iterparams, map(convert_param_to_dirname, il))
                        )
                        par["name"] = (
